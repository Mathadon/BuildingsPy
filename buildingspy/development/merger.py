...
class IBPSA(object):
    ...
    def __init__(self, ibpsa_dir, dest_dir):
        ...
        self._copFilPat = os.path.join(self._target_home, ".copiedFiles.txt")
    ...
    def filter_files(file_list, pattern):
        ...
        if '*' in pattern:
            ...
            if len(pat) is not 2:
                raise ValueError("Pattern {} is not supported.".format(pattern))
    ...
    def getPreviousMergeSha(self):
        import os

        if os.path.isfile(self._copFilPat):
            with open(self._copFilPat, mode="r", encoding="utf-8-sig") as fp:
                lines = fp.readlines()


            res = lines[-1].split("last merge: ")
            if len(res)>1:
                return res[1].strip()
    ...
    def getPreviouslyCopiedFiles(self):
        import os
    ...
    def mergeIbpsaMpc(self, files):
        ...
        # get sha and generate patch file for later
        sha = self.getPreviousMergeSha()

        # if no sha exists, we are in an ongoing merge process
        # that must be finalized
        if sha ==None:
            self.finalizeMergeIbpsaMpc()
            return
        ...
        for k,v in files.items():
            ...
            pathDest = os.path.join(self._target_home, os.sep.join(k.split(".")[1:])+".mo")
        ...
        # Iterate over the list of files to be copied.
        copiedFiles = list()
        for srcFil, opts in filesToCopy.items():
            desFil = srcFil.replace(self._ibpsa_home, self._target_home)
            desPat = os.path.dirname(desFil)
            if not os.path.exists(desPat):
                os.makedirs(desPat)
            ...
            if desFil.endswith(".mo") or desFil.endswith(".mos"):
                ...
                # Include new parameter values if required
...
2) Run the merge script again, which updates {2}, upon which the merge process continues. To abort, checkout all files.""".format(self._src_library_name, self._copFilPat, os.path.basename(self._copFilPat)))
    ...
    def finalizeMergeIbpsaMpc(self):
        import os

        sha = self.getCurrentSha()
        with open(self._copFilPat, mode="a", encoding="utf-8") as fp:
            fp.write("# Sha of last merge: {}\n".format(sha))

        print("""The merge continues. The stored sha is {0}. Next steps are:
